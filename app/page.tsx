...
<section className="worlds section-space"><div className="wrap worlds-heading"><SectionLabel>Twee richtingen. Eén basis.</SectionLabel><h2>Kies waar jouw<br/><em>beweging begint.</em></h2></div><div className="world-grid">{homeJourneys.map(journey=><a className={`world ${journey.className}`} href={journey.href} key={journey.href}><span className="world-word" aria-hidden="true">{journey.emphasis}</span><div className="world-shade"/><div className="world-index">{journey.index}</div><div className="world-copy"><p>{journey.audience}</p><h3>{journey.name} <em>{journey.emphasis}</em></h3><span>{journey.themes}</span><b>{journey.cta} <Arrow/></b></div></a>)}</div></section>
