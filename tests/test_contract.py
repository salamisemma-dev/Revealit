...
class WebsiteContract(unittest.TestCase):
    ...
    def test_homepage_visual_adjustments_are_kept_in_sync(self):
        ...
        self.assertIn('background-size:420% auto', html)
        self.assertIn('background-size:420% auto', styles)
        self.assertIn('background-size:455% auto', html)
        self.assertIn('background-size:455% auto', styles)
        self.assertIn('background-position:26% center', html)
        self.assertIn('background-position:26% center', styles)
        self.assertIn('background-position:28% center', html)
        self.assertIn('background-position:28% center', styles)
        self.assertIn('.world:before{content:"";position:absolute;z-index:2', styles)
        self.assertIn('.world-index{position:absolute;z-index:3', styles)
        self.assertIn('.world-copy{position:absolute;z-index:3', styles)
        self.assertIn('background-position:26% center;opacity:.34', html)
        self.assertIn('background-position:26% center;opacity:.34', styles)
        self.assertIn('background-position:76% center;opacity:.3', html)
        self.assertIn('background-position:76% center;opacity:.3', styles)
        self.assertIn('background-position:76% center;opacity:.32', html)
        self.assertIn('background-position:76% center;opacity:.32', styles)
        self.assertIn('-webkit-mask-image:linear-gradient(90deg,#000 0 68%,transparent 94%)', styles)
        self.assertIn('-webkit-mask-image:linear-gradient(90deg,transparent 0 4%,#000 28%)', styles)
        self.assertIn('-webkit-mask-image:linear-gradient(90deg,transparent 0 6%,#000 30%)', styles)
